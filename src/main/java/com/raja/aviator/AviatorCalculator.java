...
public class AviatorCalculator {
    ...
    public void processSimulation(String filePath) {
        ...
        float successRate = (float) (totalWonBet.get() * 49) /totalBet.get();

        System.out.println(  "Success rate : " +successRate+" , Expected Losses before a win  "+ 1/(successRate/49));
        System.out.println("=== Simulation Ended ===");
    }

    // 1. Define connection details (Adjust URL, user, and password for your DB)
    String url = "jdbc:postgresql://localhost:5432/aviator";
    String username = "readonly_uat_user";
    String password = "uat_password";

    public void processSimulationDataFromDB(LocalDateTime from, LocalDateTime to) {

        System.out.println("=== Starting Aviator Simulation ===");
        System.out.println("Initial Balance: Rs. " + balance);

        String sqlQuery = "SELECT * FROM multipliers WHERE created_on BETWEEN ? AND ? ORDER BY id ASC";
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    double actualMultiplier = resultSet.getDouble("multiplier");
